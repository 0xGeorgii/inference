...
impl Screen {
    /// Returns the screen title for display in headers.
    #[must_use]
    #[allow(dead_code)]
    pub fn title(self) -> &'static str {
        match self {
            Self::Main => "Inference Toolchain",
...
impl ToolchainsState {
    ...
    /// Returns the currently selected toolchain info, if any.
    #[must_use]
    #[allow(dead_code)]
    pub fn selected_toolchain(&self) -> Option<&ToolchainInfo> {
        self.toolchains.get(self.selected)
    }
}
...
impl ProgressItem {
    ...
    /// Creates a new progress item with a known total size.
    #[must_use]
    #[allow(dead_code)]
    pub fn with_total(description: impl Into<String>, total: u64) -> Self {
        Self {
            description: description.into(),
            current: 0,
            total,
    ...
    /// Formats the progress including speed when available.
    #[must_use]
    #[allow(dead_code)]
    pub fn format_progress_with_speed(&self) -> String {
...
impl ProgressState {
    ...
    /// Updates the progress of an item by index.
    #[allow(dead_code)]
    pub fn update_item(&mut self, index: usize, current: u64) {
        if let Some(item) = self.items.get_mut(index) {
            item.update(current);
        }
    }

    /// Marks an item as completed by index.
    #[allow(dead_code)]
...
mod tests {
    ...
    #[test]
    fn screen_titles_are_correct() {
        assert_eq!(Screen::Main.title(), "Inference Toolchain");
        assert_eq!(Screen::Toolchains.title(), "Installed Toolchains");
        assert_eq!(Screen::Doctor.title(), "Doctor Check Results");
        assert_eq!(Screen::Progress.title(), "Progress");
    }
    ...
    #[test]
    fn progress_item_percentage_with_total() {
        let mut item = ProgressItem::with_total("test", 100);
        assert!((item.percentage() - 0.0).abs() < f64::EPSILON);

        item.update(50);
        assert!((item.percentage() - 0.5).abs() < f64::EPSILON);
    ...
    #[test]
    fn progress_item_format_progress() {
        let mut item = ProgressItem::with_total("test", 1024);
        item.update(512);
        let formatted = item.format_progress();
        assert!(formatted.contains("512"));
        assert!(formatted.contains("1.0 KB"));
    }

    #[test]
    fn progress_state_overall_percentage() {
        let mut state = ProgressState::new("test");
        assert!((state.overall_percentage() - 0.0).abs() < f64::EPSILON);

        state.add_item(ProgressItem::with_total("item1", 100));
        state.add_item(ProgressItem::with_total("item2", 100));

        state.update_item(0, 50);
        state.update_item(1, 100);

        // (0.5 + 1.0) / 2 = 0.75
        assert!((state.overall_percentage() - 0.75).abs() < f64::EPSILON);
    }
    ...
    #[test]
    fn progress_item_update_with_speed() {
        let mut item = ProgressItem::with_total("test", 1000);
        item.update_with_speed(500, 1024 * 100);
        assert_eq!(item.current, 500);
        assert_eq!(item.speed_bytes_per_sec, Some(1024 * 100));
        assert!(!item.completed);

        item.update_with_speed(1000, 1024 * 200);
        assert!(item.completed);
    }

    #[test]
    fn progress_item_format_progress_with_speed_no_speed() {
        let mut item = ProgressItem::with_total("test", 1024);
        item.update(512);
        let formatted = item.format_progress_with_speed();
        assert!(formatted.contains("512"));
        assert!(formatted.contains("1.0 KB"));
    ...
    #[test]
    fn progress_item_format_progress_with_speed_has_speed() {
        let mut item = ProgressItem::with_total("test", 1024);
        item.update_with_speed(512, 1024 * 100);
        let formatted = item.format_progress_with_speed();
        assert!(formatted.contains("512"));
        assert!(formatted.contains("1.0 KB"));
