...
mod tests {
    ...
    #[test]
    fn render_with_items_does_not_panic() {
        let mut terminal = create_test_terminal();
        let theme = Theme::dark();
        let mut state = ProgressState::new("Downloading");

        state.add_item(ProgressItem::with_total("File 1", 1024));
        state.add_item(ProgressItem::with_total("File 2", 2048));
        state.update_item(0, 512);
    ...
    #[test]
    fn render_with_speed_does_not_panic() {
        let mut terminal = create_test_terminal();
        let theme = Theme::dark();
        let mut state = ProgressState::new("Downloading");

        let mut item = ProgressItem::with_total("File 1", 1024);
    ...
    #[test]
    fn render_completed_item_does_not_show_speed() {
        let mut terminal = create_test_terminal();
        let theme = Theme::dark();
        let mut state = ProgressState::new("Done");

        let mut item = ProgressItem::with_total("File 1", 1024);
        item.update_with_speed(1024, 1024 * 100);
        item.complete();
        state.add_item(item);
