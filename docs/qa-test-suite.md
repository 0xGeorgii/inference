...
- [Test Categories](#test-categories)
  ...
  - [4. Verify Command](#4-verify-command-8-tests)
...
### 4. Verify Command (8 tests)

#### TC-4.1: Basic Verification
**Priority:** Critical
**Description:** Verify complete pipeline: compile, translate, verify with coqc.

**Preconditions:**
- `trivial.inf` test file exists
- `coqc` is installed and in PATH
- `infc` compiler is available

**Steps:**
1. Run `infs verify trivial.inf`
2. Check `proofs/trivial.v` exists

**Expected Result:**
- Exit code: 0
- `.v` file generated in `proofs/` directory
- coqc verification passes

---

#### TC-4.2: Custom Output Directory
**Priority:** High
**Description:** Verify `--output-dir` places proofs in specified location.
...
**Steps:**
1. Run `infs verify trivial.inf --output-dir custom_proofs`
2. Check `custom_proofs/trivial.v` exists

**Expected Result:**
- Exit code: 0
- `.v` file in `custom_proofs/` directory
- Default `proofs/` directory not used

---

#### TC-4.3: Skip Compile Flag
**Priority:** High
**Description:** Verify `--skip-compile` uses existing .v file if up-to-date.

**Preconditions:**
- `trivial.inf` test file exists
- Previous `infs verify trivial.inf` was run successfully

**Steps:**
1. Run `infs verify trivial.inf` (first time)
2. Note modification time of `proofs/trivial.v`
3. Run `infs verify trivial.inf --skip-compile`
4. Note modification time of `proofs/trivial.v`

**Expected Result:**
- Exit code: 0
- `.v` file modification time unchanged in step 4
- Verification still runs on existing file

---

#### TC-4.4: Missing coqc Error
**Priority:** High
**Description:** Verify clear error when coqc is not available.

**Preconditions:**
- `trivial.inf` test file exists
- Ensure `coqc` is NOT in PATH
...
**Expected Result:**
- Exit code: Non-zero
- Error message indicates coqc not found
- Compilation may still succeed, error at verification step

---

#### TC-4.5: Verification Failure
**Priority:** High
**Description:** Verify handling of coqc verification failures.

**Preconditions:**
- Source file that generates unprovable .v file

**Steps:**
1. Run `infs verify unprovable.inf`

**Expected Result:**
- Exit code: Non-zero (from coqc)
- Error message indicates proof failure
- Specific coqc error is visible

---

#### TC-4.6: Compilation Error in Verify
**Priority:** High
**Description:** Verify graceful failure when compilation fails.

**Preconditions:**
- `syntax_error.inf` test file exists

**Steps:**
1. Run `infs verify syntax_error.inf`

**Expected Result:**
- Exit code: Non-zero
- Error at compilation phase
- Does not attempt verification

---

#### TC-4.7: Missing Source File
**Priority:** Medium
**Description:** Verify error for non-existent source file.

**Preconditions:**
- File `nonexistent.inf` does NOT exist

**Steps:**
1. Run `infs verify nonexistent.inf`
...
#### TC-4.8: Output Directory Creation
**Priority:** Medium
**Description:** Verify output directory is created if it doesn't exist.

**Preconditions:**
- `trivial.inf` test file exists
- Directory `new_proofs/` does NOT exist

**Steps:**
1. Run `infs verify trivial.inf --output-dir new_proofs`

**Expected Result:**
- Exit code: 0
- Directory `new_proofs/` created
- `.v` file placed inside
